...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>thanhhacklox chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "thanhhacklox", HidePremium = false, IntroText = "thanhhacklox⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("thanhhacklox SCRIPT")
W:AddLabel("Discordthanhhacklox")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "thanhhacklox(Noitification)",
